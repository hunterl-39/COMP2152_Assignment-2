...
# Stores common port numbers to their service names
common_ports = {
    21: "FTP",
    22: "SSH",
    23: "Telnet",
...
class NetworkTool:
    ...
    # using property and target.setter lets us change the target safely. We can stop empty or wrong values.
    @property
    def target(self):
...
# PortScanner gets the target variable and the checks from NetworkTool so we dont have to write them again.
...
#     Without try-except the program could stop working if a port gives and error.
...
#   Threading lets us scan multiple ports at the same time. This makes scanning faster instead of waiting for each port one by one.
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not (1 <= start_port <= 1024) or not (1 <= end_port <= 1024) or end_port < start_port:
            print("Port must be between 1 and 1024 and end port >= start port.")
            exit()
    except ValueError:
...
# I would add a feature that saves all the scan results into a text file so the user can
# save and view at any time. It would use nested if statement to check if the port is
# open before writing to the file
